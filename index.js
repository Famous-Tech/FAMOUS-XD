...
setInterval(setBio, 60000);
   ...
   /**
             ...
             */
gss.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
                let type = await gss.getFile(path, true)
                ...
                let m
                try {
                    m = await gss.sendMessage(jid, message, { ...opt, ...options })
                } catch (e) {
                    console.error(e)
                    m = null
                } finally {
                    if (!m) m = await gss.sendMessage(jid, { ...message, [mtype]: file }, { ...opt, ...options })
