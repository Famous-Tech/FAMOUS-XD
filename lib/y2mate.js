...
async function yt(url, quality, type, bitrate, server = 'en68') {
  ...
  if (table) {
    ...
    if (links) {
      list = Object.fromEntries([...links].filter(v => !/\.3gp/.test(v.innerHTML)).map(v => {
        const titleElement = v.parentElement.nextSibling;
        const linkElement = titleElement ? titleElement.nextSibling : null;
        return [v.innerHTML.match(/.*?(?=\()/)[0].trim(), linkElement ? linkElement.innerHTML : ''];
      }));
    }
  }
