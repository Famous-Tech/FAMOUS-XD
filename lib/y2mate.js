...
switch (type) {
  case 'mp4':
    let mp4Links = table.querySelectorAll('td > a[href="#"]');
    if (mp4Links.length === 0) {
      console.error('No mp4 links found');
      return null;
    }
    list = Object.fromEntries(
      [...mp4Links].filter(v => !/\.3gp/.test(v.innerHTML)).map(v => [v.innerHTML.match(/.*?(?=\()/)[0].trim(), v.parentElement.nextSibling ? v.parentElement.nextSibling.nextSibling.innerHTML : ''])
    );
    break;
  case 'mp3':
    let mp3Link = table.querySelector('td > a[href="#"]');
    if (!mp3Link || !mp3Link.parentElement || !mp3Link.parentElement.nextSibling) {
